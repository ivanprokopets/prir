...
int parseInput = 0;
  ...
  for (bytes = 0; (c = getc(f)) != EOF; bytes++) {
    ...
    if(c == ':' && parseInput == 1 && colons == 0) {
        colons = 1;
    }else if(c == ':' && parseInput == 1 && colons == 1) {
        colons = 2;
    }else if(c == ':' && parseInput == 1 && colons == 2) {
        parseInput = 0;
    }
    if (!parseInput) {
        if (!words[bytes-1]) bytes--;
...
int space = 1;
   ...
   for (bytes = 0; (c = getc(f)) != EOF; bytes++) {
    ...
    } else if (c == ' ' && parseInput == 1 && space == 1) {
        space = 0;
        newLine = 0;
		if (!words[bytes-1]) bytes--;
          continue;
	} else if (c == ' ' && parseInput == 1 && space == 0) {
        parseInput = 0;
        newLine = 0;
    } else if(c == '\n') {
		space = 1;
